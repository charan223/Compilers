	.file	"ass1_14CS10037.c"      # Name of source file is ass1_14CS10037.c
	.section	.rodata				# This implies that it is a read-only data section
	.align 8						# Align with a 8 byte boundary
.LC0:
	.string	"Enter how many elements you want:"#Label Of f-string 1st printf
...
.LC7:							    #Label Of f-string-7th printf
	.string	"%6d"
	.text	    		            #Starting of the code
	.globl	main		            #main is global name
	.type	main, @function	        #main is function
main:			    	            #main:begins
.LFB0:					            #implies the beginning of a function
	.cfi_startproc					#Call Frame Information(cfi)
	pushq	%rbp					#Save old base pointer
	.cfi_def_cfa_offset 16			#cfi_ directives are used for creation of .eh_frame to unwind stack frames for debugging
	.cfi_offset 6, -16				#and also exception handling
	movq	%rsp, %rbp				#rbp <-- rsp (Set new stack base pointer)
	.cfi_def_cfa_register 6
	subq	$432, %rsp				#Creates space for the array and also local variables
	movq	%fs:40, %rax			#fs is a file segment register , 40 is offset..
	movq	%rax, -8(%rbp)			#These 3 instructions check for the stack
	xorl	%eax, %eax				#Boundary violation
	movl	$.LC0, %edi				#edi <-- starting of format string,1st parameter
	call	puts					#call puts for printing
	leaq	-432(%rbp), %rax		#rax <-- (rbp-432)(&n)[stands for load effective address]
	movq	%rax, %rsi				#rsi <-- rax(&n,2nd parameter)
	movl	$.LC1, %edi				#edi <-- starting of format string, 1st parameter
	movl	$0, %eax				#eax <-- 0
	call	__isoc99_scanf			#calls scanf,return value is in eax
	movl	-432(%rbp), %eax		#eax <-- (rbp-432)[n]
	movl	%eax, %esi				#esi <-- eax(n,2nd parameter)
	movl	$.LC2, %edi				#edi <-- starting of format string,1st parameter
	movl	$0, %eax				#eax <-- 0
...
.L3:
	...
	movl	-424(%rbp), %edx		#edx <-- (rbp-424)[copy value of i to edx]
	movslq	%edx, %rdx			    #conversion of 32 bit to 64 bit(sign extended)
...
.L2:
	...
	cmpl	%eax, -424(%rbp)		#compare value to value of i
	...
	movl	$.LC3, %edi			   	#edi <-- starting address of printf format string,1st parameter
	call	puts				   	#call puts for printing
	leaq	-428(%rbp), %rax		#[Load Effective Address] rax <-- (rbp-428) (&item)
	movq	%rax, %rsi			    #rsi <-- rax(&item,2nd parameter)
	movl	$.LC1, %edi			    #starting address of format string , 1st parameter
	movl	$0, %eax			    #eax <-- 0
	call	__isoc99_scanf			#calls scanf, return value is in eax
...
.LFE2:						    	#function ending
	...
	.type	insert, @function		#insert is function
insert:
.LFB3:						    	#starting of function
	.cfi_startproc
	pushq	%rbp					#Save old base pointer
	.cfi_def_cfa_offset 16
	.cfi_offset 6, -16
...
.L25:
	...
	subl	$1, -4(%rbp)		    #i <-- i - 1
.L23:
	cmpl	$0, -4(%rbp)		    #comparing of i with 0
	js	.L24					    #jumping to L24 if (!(i >= 0))
	movl	-4(%rbp), %eax		    #eax <-- i
	cltq						    #conversion of 32 bit to 64 bit
	leaq	0(,%rax,4), %rdx	    #rdx <-- rax*4[load effective address]
	movq	-24(%rbp), %rax	        #rax <-- (rbp-24) [&num[0]]
	...
	movl	(%rax), %eax		    #eax <-- num[i]
	cmpl	-32(%rbp), %eax		    #comparing k with num[i]
	jg	.L25					    #jumping to L25 if num[i] > k
.L24:
	movl	-4(%rbp), %eax		    #eax <-- i
	cltq						    #conversion from 32 bit to 64 bit
	addq	$1, %rax			    #rax <-- rax + 1
	...
	ret			    		     	#Returning
	.cfi_endproc
.LFE3:				    			#Ending of the function
	.size	insert, .-insert
	.ident	"GCC: (Ubuntu 5.4.1-13ubuntu2) 5.4.1"
